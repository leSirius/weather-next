
export const AirIcon = function ({width, color = 'white'}){
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={`${width}`} height={`${width}`} fill={`${color}`} className="qi-1067" viewBox="0 0 16 16">
      <path d="M12 .5a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM14 3a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm2 1.5a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0Zm-1-3a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM1.5 2a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0Zm5-1a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM3 15a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm11.134-.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0ZM4 13.5a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM1 13a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0Zm10.5.75a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm-6 .75a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0Zm10.5 0a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0Zm-.5-1.5a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM4.5 1.75a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM2.5 4a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm9.227 7.217A4.99 4.99 0 0 1 7.9 13a4.988 4.988 0 0 1-3.773-1.719 3 3 0 1 1-.586-5.732A4.998 4.998 0 0 1 7.9 3a4.999 4.999 0 0 1 4.38 2.587 3 3 0 1 1-.553 5.63ZM7.605 5.605c-.326.197-.553.635-.496.997L7.454 8.8h1.078l.358-2.277a.846.846 0 0 0-.408-.853l-.07-.042c-.256-.156-.55-.176-.807-.023ZM8 10.5a.6.6 0 1 0 0-1.2.6.6 0 0 0 0 1.2Z"/>
    </svg>
  )
}


export const IndicesIcon = function ({width, color='white'}){
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={`${width}`} height={`${width}`} fill={`${color}`} className="qi-1082" viewBox="0 0 16 16">
      <path d="M5.109 4.132c-.057-.362.17-.8.496-.997.256-.153.551-.133.806.023l.07.042a.846.846 0 0 1 .409.853L6.532 6.33H5.454L5.11 4.132ZM6.6 7.43a.6.6 0 1 1-1.2 0 .6.6 0 0 1 1.2 0Z"/>
      <path d="M7.178 0c1.097 0 2.136.21 3.063.584 2.452.97 4.15 3.083 4.15 5.536v1.014l1.462 2.378c.37.6.019 1.092-.774 1.092h-.689v2.26c0 .705-.677 1.286-1.508 1.286l-2.071-.263-.004 1.765v.013c-.011.226-.521.342-.512.335h-7.97v-5.38C.897 9.502 0 7.899 0 6.114.004 2.735 3.217 0 7.178 0ZM10 5.53a4 4 0 1 0-8 0 4 4 0 0 0 8 0Z"/>
    </svg>
  )
}


export const ForecastIcon = function ({width, color='white'}){
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={`${width}`} height={`${width}`} fill={`${color}`} className="qi-1085" viewBox="0 0 16 16">
      <path d="M2.5 1.866a1 1 0 1 0-1 0V15H0v1h4v-1H2.5V4.292l2.13 1.277c.046-.129.105-.298.161-.486L2.986 4 4.79 2.917a9.43 9.43 0 0 0-.162-.486L2.5 3.708V1.866Z"/>
      <path d="M5.698 2.464c.346-.015.8-.032 1.302-.042V5.01c-.46.1-.882.198-1.227.281l.048-.171C5.914 4.775 6 4.364 6 4c0-.383-.096-.82-.195-1.18a9.243 9.243 0 0 0-.107-.356ZM8 2.414c.509.002 1.026.015 1.5.047v2.094c-.477.063-.992.151-1.5.25V2.413Zm2.5.15c.478.067.995.159 1.5.26v1.59a26.44 26.44 0 0 0-1.5.045V2.564Zm2.5.47c.62.137 1.16.27 1.5.354v1.084c-.368-.017-.9-.039-1.5-.05V3.034ZM5.11 1.493a.601.601 0 0 0-.527.814c.06.159.167.454.258.781.093.337.159.668.159.912 0 .23-.058.54-.145.86a9.06 9.06 0 0 1-.251.778c-.166.444.23.92.713.796.706-.18 3.118-.775 4.733-.936 1.693-.17 4.26-.037 4.817-.005a.6.6 0 0 0 .633-.6V3.077a.6.6 0 0 0-.449-.58c-.47-.122-3.184-.813-5.001-.995-1.75-.175-4.328-.044-4.94-.01Zm7.841 8.458c-.064 0-.106-.06-.077-.112l.95-1.726c.029-.052-.013-.113-.077-.113h-1.894a.176.176 0 0 0-.092.024.19.19 0 0 0-.069.068l-1.677 3.165c-.05.094.027.203.142.203h1.705c.06 0 .1.053.082.104l-.886 2.332c-.034.083.086.142.148.076l3.574-3.892c.046-.05.007-.129-.066-.129H12.95Zm-7.365 3.463A2 2 0 0 1 5 12c0-1 1.11-2.79 2-4 .89 1.21 2 3 2 4a2 2 0 0 1-3.414 1.414Z"/>
    </svg>
  )
}

export function CompassPlate({width}) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} viewBox="0 0 100 100">
      <path fill="#fcf9e8" d="M49.95 0a1.518 1.518 0 0 0-1.022.443L45.69 3.68A46.317 46.317 0 0 0 20.31 14.2h-4.594a1.518 1.518 0 0 0-1.108.48a1.518 1.518 0 0 0-.41 1.037v4.594A46.317 46.317 0 0 0 3.68 45.69L.444 48.928a1.518 1.518 0 0 0 0 2.146l3.237 3.237A46.317 46.317 0 0 0 14.2 79.689v4.594a1.518 1.518 0 0 0 1.517 1.518h4.594a46.317 46.317 0 0 0 25.378 10.52l3.237 3.236a1.518 1.518 0 0 0 2.146 0l3.235-3.237A46.317 46.317 0 0 0 79.689 85.8h4.592a1.518 1.518 0 0 0 1.52-1.517l-.002-4.592A46.316 46.316 0 0 0 96.32 54.307l3.237-3.235a1.518 1.518 0 0 0 0-2.146l-3.237-3.237A46.317 46.317 0 0 0 85.8 20.31l.002-4.592a1.518 1.518 0 0 0-1.52-1.518H79.69A46.317 46.317 0 0 0 54.311 3.68L51.074.444A1.518 1.518 0 0 0 49.95 0m-4.712 8.752l3.69 3.69a1.518 1.518 0 0 0 2.144 0l3.688-3.69a41.312 41.312 0 0 1 21.039 8.7v5.232a1.518 1.518 0 0 0 1.52 1.517h5.23a41.312 41.312 0 0 1 8.699 21.037l-3.69 3.69a1.518 1.518 0 0 0 0 2.146l3.69 3.69a41.313 41.313 0 0 1-8.7 21.035h-5.23a1.518 1.518 0 0 0-1.109.482a1.518 1.518 0 0 0-.41 1.035v5.233a41.313 41.313 0 0 1-21.035 8.699l-3.69-3.69a1.518 1.518 0 0 0-1.125-.443a1.518 1.518 0 0 0-1.021.444l-3.69 3.689A41.312 41.312 0 0 1 24.2 82.547v-5.23a1.518 1.518 0 0 0-1.515-1.518H17.45a41.312 41.312 0 0 1-8.699-21.04l3.69-3.687a1.518 1.518 0 0 0 0-2.144l-3.69-3.69a41.312 41.312 0 0 1 8.7-21.037h5.232a1.518 1.518 0 0 0 1.515-1.517v-5.23a41.312 41.312 0 0 1 21.04-8.702M50 46.561a3.44 3.44 0 1 0 0 6.878a3.44 3.44 0 0 0 0-6.878" color="white"></path>
    </svg>
  )
}
export function CompassPointer({width}) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} viewBox="0 0 100 100">
      <path fill="#fcf9e8" d="M50.03 5a2.516 2.516 0 0 0-2.43 1.76L34.493 48.548a2.51 2.51 0 0 0-.372 1.454c-.026.51.104 1.017.372 1.452l13.105 41.782c.737 2.352 4.065 2.352 4.802 0l13.105-41.785c.27-.436.399-.945.372-1.456a2.513 2.513 0 0 0-.372-1.45L52.401 6.76A2.513 2.513 0 0 0 50.03 5M39.403 50.288h6.205c.152 2.306 2.048 4.134 4.392 4.134c2.344 0 4.24-1.828 4.392-4.134h6.461L50 84.078Z" color="white"></path>
    </svg>
  )
}

export function moonIcons(name, width, color='white') {
  const obList = {
    '800':  (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className='qi-800 ' viewBox="0 0 16 16">
        <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16Z"/>
      </svg>
    ),
    '801': (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-801" viewBox="0 0 16 16">
        <path d="M9 0c-.265 0-.53.013-.795.04a7.985 7.985 0 0 0-.631.094c-.043.008-.087.011-.13.02a7.998 7.998 0 0 0 0 15.692c.043.009.087.012.13.02.208.037.417.073.631.094A8 8 0 1 0 9 0Zm0 15.5a7.46 7.46 0 0 1-1.668-.188 8.497 8.497 0 0 0 0-14.623A7.5 7.5 0 1 1 9 15.5Z"/>
      </svg>
    ),
    802:(
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-802" viewBox="0 0 16 16">
        <path d="M8 0c-.356.003-.712.03-1.065.079a7.992 7.992 0 0 0-.354 15.788h.001c.468.087.942.131 1.418.133A8 8 0 0 0 8 0Zm0 15.5a6.76 6.76 0 0 1-.725-.04A24.01 24.01 0 0 0 8.5 7.5 25.67 25.67 0 0 0 7.593.514C7.734.504 7.868.5 8 .5a7.5 7.5 0 1 1 0 15Z"/>
      </svg>
    ),
    803: (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-803" viewBox="0 0 16 16">
        <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0Zm0 15a6.73 6.73 0 0 1-.948-.072.486.486 0 0 1-.24-.106A8.838 8.838 0 0 1 3.962 8 8.868 8.868 0 0 1 6.76 1.22a.702.702 0 0 1 .359-.157c.292-.04.586-.062.881-.063a7 7 0 0 1 0 14Z"/>
      </svg>
    ),
    804: (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-804" viewBox="0 0 16 16">
        <path d="M8 1a7 7 0 1 1-7 7 7.008 7.008 0 0 1 7-7m0-1a8 8 0 1 0 0 16A8 8 0 0 0 8 0Z"/>
      </svg>
    ),
    805: (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-805" viewBox="0 0 16 16">
        <path d="M0 8a8 8 0 1 0 16 0A8 8 0 0 0 0 8Zm1 0a7.008 7.008 0 0 1 7-7c.295.001.59.022.881.063a.702.702 0 0 1 .36.157A8.868 8.868 0 0 1 12.036 8a8.838 8.838 0 0 1-2.849 6.822.486.486 0 0 1-.24.106A6.73 6.73 0 0 1 8 15a7.008 7.008 0 0 1-7-7Z"/>
      </svg>
    ),
    806: (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-806" viewBox="0 0 16 16">
        <path d="M8 0a8.03 8.03 0 0 0-1.065.079 7.992 7.992 0 0 0-.354 15.788h.001c.468.087.942.131 1.418.133A8 8 0 0 0 8 0ZM1 8a7.008 7.008 0 0 1 6.204-6.951A25.25 25.25 0 0 1 8 7.5c.032 2.51-.328 5.01-1.067 7.41A7.005 7.005 0 0 1 1 8Z"/>
      </svg>
    ),
    807: (
      <svg xmlns="http://www.w3.org/2000/svg" width={width} height={width} fill={color} className="qi-807" viewBox="0 0 16 16">
        <path d="M10.555.154c-.042-.009-.085-.012-.129-.02A7.981 7.981 0 0 0 9.795.04a8 8 0 1 0 0 15.92 8.12 8.12 0 0 0 .631-.094c.043-.008.087-.011.13-.02a7.998 7.998 0 0 0 0-15.692h-.001Zm.113 15.158a7.499 7.499 0 1 1 0-14.623 8.497 8.497 0 0 0 0 14.623Z"/>
      </svg>
    )
  }
  return obList[name];
}